...
function calculate() {
  ...
  if (isNaN(distance) || isNaN(velocityA) || isNaN(velocityB)) {
  ...
  document.getElementById(
    "result"
  ).textContent = `${distance}${d_result} ÷ (${velocityA}${v_result}- ${velocityB}${v_result}) = ${(
    distance /
    (velocityA - velocityB)
  ).toFixed(2)} ${t_unit}後`;
...
function draw() {
  ...
  if (animationStart) {
    ...
    if (posA - 1 >= posB) {
      ...
      textSize(20);
      text(
        (distance / (velocityA - velocityB)).toFixed(2) + "" + t_unit + "後",
        posA - 20,
        160
      );
      noLoop();
